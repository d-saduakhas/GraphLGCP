...
In this section, we fit **Model 2** as described in the paper. This model includes spatial effects and covariates. The details on the covariates used in the analysis are provided in the [Extracting Covariates](covariates.html)  and  [Computing Spatial Covariates](covariates2.html)  sections.
...
The final model that we report in the paper is `spde_fit2_5`. We can now extract the results of the model and plot the intensity and latent field. The function `plot_and_save_results` is defined below to plot and save the results.
```{r}
...
library(RColorBrewer)
 ...
 plot_and_save_results(spde_fit2_5, graph, stk2, "Figures/5Graph_model2_alpha1_7FRC_pruned_exact_100m")
