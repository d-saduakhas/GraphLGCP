...
In this section, we extract the locations of spatial covariates from OpenStreetMap data. We collect the positions of mosques, educational institutions, hospitals, financial institutions (banks and ATMs), and road intersections within the study area. The distance computations and transformations are covered in the next section ([Computing Spatial Covariates](covariates2.html)).
...
# Load graph
graph <- qread(here("Data/graph_mesh_pruned_50m_7FRC.qs"))
```
...
data_building <- opq(bb = bb,timeout = 300) %>%
...
fetch_osm_data <- function(key, value, bb) {
 ...
 # combined <- combined %>% filter(!!sym(key) == value)
  combined <- combined %>% filter(!!sym(key) == value | is.na(!!sym(key)))
...
education_places = remove_duplicates_within_threshold(education, 300)
